# gaitSpeed_2D (36)process.py
# It processes videos recorded by gaitSpeed_2D (33)record.py
...
ap.add_argument('picklePath', type=str, nargs="?", default="", \
    help="1st: Recording data needed for processing")
ap.add_argument('leftVideo', type=str, nargs="?", default="", \
    help="2nd: Recorded video file from left camera")
ap.add_argument('rightVideo', type=str, nargs="?", default="", \
    help="3rd: Recorded video file from right camera")
args = ap.parse_args()
picklePath = args.picklePath
leftVideo = args.leftVideo
rightVideo = args.rightVideo
print ('\npicklePath =', picklePath)
print ('leftVideo =', leftVideo)
print ('rightVideo =', rightVideo)
...
dataList, correctDist, timeLimit = dataLoad[0],dataLoad[1],dataLoad[2]
fast, pattern, height = dataLoad[3],dataLoad[4],dataLoad[5]
width, doctor, patientID = dataLoad[6],dataLoad[7],dataLoad[8]
fps, t_raw_, recordTime = dataLoad[9],dataLoad[10],dataLoad[11]
recordFrames, record_fps, stamp = dataLoad[12],dataLoad[13],dataLoad[14]
setupTime = dataLoad[15]
...
big = 3 # needed for template matching
...
cap0 = cv2.VideoCapture(leftVideo)
cap2 = cv2.VideoCapture(rightVideo)
fourcc = cv2.VideoWriter_fourcc(*'XVID')
processVideos025 = '/home/pi/process/gaitSpeedProcessed/'+ stamp + ' processVideo.avi'
if pattern == 1:
    less = 0
    less2 = 0
...
while 1:
    ...
    if not success0:
        ...
        cv2.putText(frame0,'FAIL fc2={}'.format(fc2),
                    (0,cue02), font, fsize2, red, fBold2, LT)
        showWrite025()
...
elapsedTime = t_[end_i]-t_[start_i]
distance = movCum_[end_i]
aveSpeed = distance/elapsedTime
distanceFromSpeed = distance_from_speed_(t_[start_i:end_i], speed_[start_i:end_i])
...
for fc in range(len(t_raw2_)-1):      
    ...
    fcsv.write('{:.1f},'.format(z0_raw_[fc]))
    fcsv.write('{:.1f},'.format(z2_raw_[fc]))
...
runStr1 = '/home/pi/process/gaitSpeedProcessed/' + stamp + ' ' + runZfill + ' Fig 1.png'
runStr2 = '/home/pi/process/gaitSpeedProcessed/' + stamp + ' ' + runZfill + ' Fig 2.png'
runStr3 = '/home/pi/process/gaitSpeedProcessed/' + stamp + ' ' + runZfill + ' Fig 3.png'
runStr4 = '/home/pi/process/gaitSpeedProcessed/' + stamp + ' ' + runZfill + ' Fig 4.png'
...
totalTime = setupTime + recordTime + processTime
print ('totalTime = {:.3f} s'.format(totalTime))
